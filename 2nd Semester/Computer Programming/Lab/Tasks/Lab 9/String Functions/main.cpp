...
using namespace std;
const int SIZE=5;
int strlen(char A[SIZE]) //strlen Function Definition
{
    int i=0;
    for(;A[i]!='\0';i++);
    return i;
}
void strcmp(char A[SIZE],char B[SIZE]) //strcmp Function Definition
{
    int i=0;
    for(;A[i]!='\0' && B[i]!='\0';i++)
    {
...
void strcat(char A[SIZE],char B[SIZE]) //strcat Function Definition
{
    int lA=strlen(A);
    int i=lA;
...
void strrev(char A[SIZE]) //strrev Function Definition
{
    for(int i=strlen(A)-1;i>=0;i--)
        cout<<A[i];
}
void strcpy (char A[SIZE],char B[SIZE]) //strcpy Function Definition
{
    int i=0;
    for(;B[i]!='\0';i++)
    {
...
void Tolower(char A[SIZE]) //Tolower Function Definition
{
    for(int i=0;A[i]!='\0';i++)
    {
        if (A[i]>='A'&&A[i]<='Z')
...
void Toupper(char A[SIZE]) //Toupper Function Definition
{
    for(int i=0;A[i]!='\0';i++)
    {
        if (A[i]>='a'&&A[i]<='z')
